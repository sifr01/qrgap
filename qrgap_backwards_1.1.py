...
security purposes (eg. a 4096 byte key from PC to smartphone)
This version of the script displays the qrstream segments in reverse order
so that the qrcodes can be scanned and then closed in the correct order'''

import qrcode
import Image

FILE_LOCATION = raw_input("Specify directory of text file (eg. /tmp/key.asc): ")
#FILE_LOCATION = "/home/username/key.asc"			#for debugging purposes
...
for INDEX, CHUNK in reversed(list(enumerate(ARRAY))):	#cycle backwards through the array elements
#for (INDEX,CHUNK) in enumerate(ARRAY):			#cycle forwards through the array elements
	...
	qr.add_data(CHUNK)				#inputs text segments/chunks to the qrcode object (qr)
